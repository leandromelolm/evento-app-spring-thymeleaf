...
public class WebSecurityConfig {
	...
	protected SecurityFilterChain filterChain(HttpSecurity http) throws Exception{ // 1-ADMIN, 2-POWER_USER, 3-STANDARD_USER (Adicionados no data.sql)
		http
			...
			.antMatchers(HttpMethod.GET, "/user/meus-eventos/deletarEvento").hasRole("ADMIN")
	...
	public WebSecurityCustomizer webSecurityCustomizer() {
		return (web) -> web.ignoring().antMatchers("/bootstrap-5.2.0-dist/**","/jquery-3.6.0-dist/**","/materialize/**", "/style/**", "/js/**","/css/**","/h2-console/**","/webjars/**", "/jquery.mask/**");
	}	
